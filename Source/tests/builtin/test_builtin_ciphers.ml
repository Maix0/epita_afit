...
let str2list_f str f =
  let rec __str2list i =
    if i >= String.length str then [] else f str.[i] :: __str2list (i + 1)
  in
  __str2list 0

let str2list str = str2list_f str (function x -> Char.code x)
...
let encrypt_cesar_tests () =
  ...
  and do_check ((k, m, b), expected) =
    check (list int)
      (sprintf "encrypt_cesar: key: %i, word: %s, base: %i" k
         (string_of_intlist m) b)
      expected (encrypt_cesar k m b)
  in
...
let decrypt_cesar_tests () =
  ...
  and do_check ((k, m, b), expected) =
    check (list int)
      (sprintf "decrypt_cesar: key: %i, word: %s, base: %i" k
         (string_of_intlist m) b)
      expected (decrypt_cesar k m b)
  in
  List.iter do_check cases
...
let generate_keys_rsa_tests () =
  ...
  and do_check ((p, q), expected) =
    ...
    check bool
      (sprintf
         "generate_rsa_keys: with %i and %i (we check if e is the inverse of d \
          modulo n)"
         p q)
      true
...
let encrypt_rsa_tests () =
  ...
  and do_check ((m, (n, e)), expected) =
    check int
      (sprintf "encrypt_rsa: %i with n=%i and e=%i" m n e)
      expected
      (encrypt_rsa m (n, e))
  in
  List.iter do_check cases
...
let decrypt_rsa_tests () =
  ...
  and do_check ((m, (n, e)), expected) =
    check int
      (sprintf "decrypt_rsa: %i with n=%i and e=%i" m n e)
      expected
      (decrypt_rsa m (n, e))
  in
  List.iter do_check cases
...
let el_gamal_tests () =
  ...
  and do_check (p, msg) =
    ...
    check int
      (sprintf
         "el_gamal functions: keys generated with p=%i, message %i is \
